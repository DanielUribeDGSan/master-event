...
document.getElementsByClassName("scrollable-area")[0].addEventListener(
  ...
  function () {
    let scrollTop =
      document.getElementsByClassName("scrollable-area")[0].scrollTop;
    const scrollHeight =
      document.getElementsByClassName("scrollable-area")[0].scrollHeight; // added
    const offsetHeight =
      document.getElementsByClassName("scrollable-area")[0].offsetHeight;
                            ...
                            <div class="chat-line__no-background tw-inline">
                                <div class="tw-inline-block"><i class="fas fa-user-shield userChatAdmin" ></i>
                                    <span class="chat-line__username" role="button" tabindex="0"><span><span class="chat-author__display-name" data-a-target="chat-message-username userChatAdmin"
                                              data-a-user="danieluribedg" data-test-selector="message-username"><strong>${
                                                doc.nombre
                                              }</strong></span></span>
                                    </span>
                                </div><span aria-hidden="true" data-test-selector="chat-message-separator">:
                                </span><span class="text-fragment"
                                  data-a-target="chat-message-text"><span class="msg userColorChatAdmin fondoChatAdmin">${doc.mensaje.replace(
                                    "#ca9e67",
                                    "#ffffff"
                                  )}</span></span>
                            </div>
